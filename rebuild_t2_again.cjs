        ...
        <table className="w-full text-center border-collapse text-[10px]">
          <thead>
            <tr className="text-white" style={{ backgroundColor: themeColor }}>
              <th className="p-1.5 font-medium w-[40px]">SN</th>
              <th className="p-1.5 font-medium text-left">Item Name</th>
              <th className="p-1.5 font-medium w-[70px]">HSN/SAC</th>
              <th className="p-1.5 font-medium w-[50px]">Qty</th>
              <th className="p-1.5 font-medium w-[70px]">Rate</th>
              <th className="p-1.5 font-medium w-[60px]">Dis.</th>
              <th className="p-1.5 font-medium w-[90px] text-right">Taxable</th>
              <th className="p-1.5 font-medium w-[100px] text-right">Amount</th>
            </tr>
          </thead>
          <tbody>
            {parsedItems.map((item, idx) => (
              <tr key={idx} className="border-b border-gray-200">
                ...
                <td className="p-1.5 text-left font-bold">{item.name}</td>
                <td className="p-1.5">{item.hsn || '-'}</td>
                <td className="p-1.5">{item.qty}</td>
                <td className="p-1.5">₹ {Number(item.rate).toFixed(2)}</td>
                <td className="p-1.5">₹ {Number(item.discount).toFixed(2)}</td>
                <td className="p-1.5 text-right">₹ {Number(item.taxableValue).toFixed(2)}</td>
                <td className="p-1.5 text-right">₹ {Number(item.totalAmount).toFixed(2)}</td>
              </tr>
            ))}
            {/* Empty padding rows */}
            <tr className="h-[40px]">
            ...
            <tr className="font-bold border-b border-t border-gray-300">
              <td colSpan="3" className="p-1.5 text-center">Total</td>
              <td className="p-1.5">{totalQty.toFixed(2)}</td>
              <td colSpan="2" className="p-1.5"></td>
              <td className="p-1.5 text-right">₹ {totalTaxable.toFixed(2)}</td>
              <td className="p-1.5 text-right text-[12px]" style={{ color: themeColor }}>₹ {totalFinal.toFixed(2)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="w-full mt-4 flex flex-col">
        ...
        <div className="w-full px-2 mb-4">
          <table className="w-full text-center border-collapse text-[10px]">
            <thead>
              <tr className="text-white" style={{ backgroundColor: themeColor }}>
                <th className="p-1 font-medium w-[40px]">SN</th>
                <th className="p-1 font-medium text-left px-2">HSN/SAC</th>
                <th className="p-1 font-medium">Taxable Amount</th>
                <th className="p-1 font-medium">GST(%)</th>
                <th className="p-1 font-medium">CGST</th>
                <th className="p-1 font-medium">SGST</th>
                <th className="p-1 font-medium">IGST</th>
                <th className="p-1 font-medium">Total Tax</th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-b border-gray-200 text-[10px]">
                <td className="p-1.5">1</td>
                <td className="p-1.5 text-left px-2">5407</td>
              ...
              <tr className="font-bold border-b border-gray-300 text-[10px]">
        ...
        <div className="w-full bg-[#f8f9fc] flex flex-col p-4 mt-auto relative min-h-[220px]">
...
console.log('Template2 rebuilt again');
