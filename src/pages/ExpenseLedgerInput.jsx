...
export function ExpenseLedgerInput() {
  ...
  const filteredExpenses = expenses.filter(exp => exp.name.toLowerCase().includes(expenseSearch.toLowerCase()));

  useEffect(() => {
    setHighlightedIndex(filteredExpenses.length > 0 ? 0 : -1);
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'ArrowDown') {
      ...
      setHighlightedIndex(prev => {
        const next = prev + 1;
        return next >= filteredExpenses.length ? 0 : next;
      });
      ...
      setHighlightedIndex(prev => {
        const next = prev - 1;
        return next < 0 ? filteredExpenses.length - 1 : next;
      });
    } else if (e.key === 'Enter') {
      e.preventDefault();
      if (highlightedIndex >= 0 && highlightedIndex < filteredExpenses.length) {
        handleSelectExpense(filteredExpenses[highlightedIndex]);
      }
               ...
               <div className="flex justify-between items-center px-1">
                 ...
                 {isAccountWise && (
                   <span className="text-[13px] font-bold text-[#dc3545]">
                     Account Balance : ₹{(selectedExpense?.balance || 0).toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})}
                   </span>
                 )}
               </div>
                        ...
                        <div 
                          ...
                          className={`p-2 border-b transition-colors cursor-pointer flex justify-between ${
                            isHighlighted 
                              ...
                              : (selectedExpense?.id === exp.id ? 'bg-[#add8e6] border-gray-200' : 'bg-white border-gray-150')
                        ...
                        >
                          ...
                          <div className="flex flex-col items-end justify-between">
                            ...
                            <div className="flex gap-2 mt-1">
                              <Edit2 className="w-3.5 h-3.5 text-[#4F46E5] hover:text-cyan-700" onClick={(e) => handleEditExpense(e, exp.name)} />
