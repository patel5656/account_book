...
export function CustomerSelectDropdown({ customers, value, customValue, onChange, onCustomChange, onEdit, onDelete, searchMode = 'Customer Name', allowCustom = false }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      const isOutsideMenu = !event.target.closest('.customer-dropdown-menu');
      
      if (isOutsideTrigger && isOutsideMenu) {
        if (allowCustom && onCustomChange && searchTerm.trim() && !value && !customValue) {
    ...
    const handleScroll = (event) => {
      if (event.target && event.target.closest && event.target.closest('.customer-dropdown-menu')) {
        return; 
      }
  ...
  }, [allowCustom, onCustomChange, searchTerm, value, customValue]);
  ...
  const selectedcustomer = customers.find(s => String(s.id) === String(value)) || null;

  const filteredcustomers = customers.filter(s => {
  ...
  useEffect(() => {
    itemRefs.current = itemRefs.current.slice(0, filteredcustomers.length);
  }, [filteredcustomers]);
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      if (filteredcustomers.length === 0 && searchTerm.trim()) {
        if (allowCustom && onCustomChange) {
           onCustomChange(searchTerm);
           setIsOpen(false);
        } else {
           onEdit({ name: searchTerm, type: 'CUSTOMER', status: 'Active', balance: 0, partyType: 'customer' });
           setIsOpen(false);
        }
      } else if (highlightedIndex >= 0 && highlightedIndex < filteredcustomers.length) {
        handleSelect(filteredcustomers[highlightedIndex].id);
      } else if (allowCustom && onCustomChange && searchTerm.trim()) {
        onCustomChange(searchTerm);
        setIsOpen(false);
      }
  ...
  return (
    <div className="relative w-full h-full flex items-center" ref={dropdownRef}>
      <div 
        className="flex-1 h-full border border-gray-300 rounded-[3px] px-3 py-1.5 text-[13px] outline-none font-bold text-gray-800 bg-[#e3f2fd] flex items-center justify-between cursor-pointer hover:border-blue-400 min-w-0"
        onClick={() => setIsOpen(!isOpen)}
      >
        <span className="truncate flex-1 text-left">
          {selectedcustomer ? selectedcustomer.name : (customValue ? customValue : <span className="text-gray-500 font-normal">Select customer...</span>)}
        </span>
      ...
      {isOpen && menuRect && createPortal(
        <div 
          className="customer-dropdown-menu bg-white border border-gray-300 shadow-2xl rounded-[3px] z-[9999] flex flex-col max-h-[200px]"
          style={{
            position: 'fixed',
          ...
          <div className="overflow-y-scroll flex-1 pos-product-scroll">
            {filteredcustomers.length === 0 ? (
              <div className="p-4 text-center flex flex-col items-center justify-center gap-2">
                <span className="text-sm text-gray-500 font-medium">No customers found</span>
                ...
                {searchTerm.trim() && allowCustom && (
                  <button 
                    ...
                    className="bg-[#17a2b8] hover:bg-[#138496] text-white px-3 py-1.5 rounded-[3px] text-[12px] font-bold transition-colors shadow-sm"
                  >
                    Use "{searchTerm}" as one-time
                  </button>
                )}
              </div>
            ) : (
              filteredcustomers.map((s, index) => {
