    ...
    <div className="w-full bg-white text-black p-4 text-[10px] relative font-sans">
      ...
      <div className="w-full flex justify-between items-start mb-4 relative z-10">
        <div className="flex-[1.5]">
          ...
          <div className="text-[18px] font-bold text-gray-700 leading-tight uppercase mt-2">
            {previewInvoice?.companyName || 'SWAYAM BILLING SOFTWARE'}
          </div>
        ...
        <div className="flex-[2] flex flex-col items-end">
           ...
           >
             <div className="flex-1 text-[11px] leading-tight font-semibold border-r border-white/30 pr-4">
                <div>{previewInvoice?.companyEmail || 'swayamsoftwaretarget@gmail.com'}</div>
                <div>Phone No: {previewInvoice?.companyPhone || '9845972853'}</div>
             </div>
             <div className="flex-[1.5] text-[10px] leading-tight font-semibold">
                {previewInvoice?.companyAddress || 'NO., OPP GRAM PANCHAYAT, SH 31, BELAGAVI, KARNATAKA, INDIA - 591220'}
             </div>
           </div>
      ...
      <div className="w-full flex gap-4 mb-4">
        ...
        <div className="flex-1 flex flex-col gap-1 border-r border-gray-200 pr-4">
          ...
          <div className="font-bold uppercase text-[11px]">{previewInvoice?.customerName || 'NISHIT'}</div>
          ...
          <div className="uppercase w-3/4">
             {previewInvoice?.customerAddress || 'A-406, 4TH FLOOR, MONARCH GAURAVPATH ROAD, PAL, BAMBOO FLAT, ANDAMAN AND NICOBAR ISLANDS, INDIA'}
          </div>
        ...
        <div className="flex-[0.8] flex flex-col gap-1 justify-end pb-4 font-bold">
           <div className="grid grid-cols-[100px_1fr] gap-2">
             <div>Invoice No.</div>
             <div>: {previewInvoice?.invoiceNumber || 'MA22/2348'}</div>
           </div>
           <div className="grid grid-cols-[100px_1fr] gap-2">
             <div>Invoice Date</div>
             <div>: {previewInvoice?.invoiceDate || '24-08-2023'}</div>
           </div>
           <div className="grid grid-cols-[100px_1fr] gap-2">
             <div>Place of Supply</div>
             <div className="uppercase">: {previewInvoice?.placeOfSupply || 'GUJARAT'}</div>
           </div>
        </div>
      </div>
      ...
      <div className="w-full flex gap-4 mb-4 pt-4 border-t border-gray-300">
        ...
        <div className="flex-1 flex flex-col gap-1 border-r border-gray-200 pr-4">
          ...
          <div className="font-bold uppercase text-[11px]">{previewInvoice?.shippingName || previewInvoice?.customerName || 'NISHIT'}</div>
          {previewInvoice?.shippingGst && <div><span className="font-bold">GSTIN:</span> {previewInvoice.shippingGst}</div>}
          <div className="uppercase w-3/4">
             {previewInvoice?.shippingAddress || previewInvoice?.customerAddress || 'A-406, 4TH FLOOR, MONARCH GAURAVPATH ROAD, PAL, BAMBOO FLAT, GUJARAT, INDIA'}
          </div>
        ...
        <table className="w-full text-center border-collapse">
          ...
          <tbody>
            {parsedItems && parsedItems.length > 0 ? (
              ...
              [1, 2, 3, 4].map((i) => (
                <tr key={i} className="border-b border-gray-300 last:border-b-0">
                  <td className="p-2 border-r border-gray-300">{i}</td>
                  <td className="p-2 border-r border-gray-300 text-left">
                     {i === 1 && 'Adrian Bell'}
                     {i === 2 && <div>Keyboard Lenovo<div className="text-gray-500 text-[9px] italic">(A keyboard is an input device...)</div></div>}
                     {i === 3 && 'Accounting Service'}
                     {i === 4 && 'Cricket Bat'}
                  </td>
                  <td className="p-2 border-r border-gray-300">{i > 1 ? '5407' : ''}</td>
                  <td className="p-2 border-r border-gray-300">{i === 3 ? 5 : 10}</td>
                  <td className="p-2 border-r border-gray-300">{i===1?'₹14.29':i===2?'₹150.00':i===3?'₹1,126.00':'₹120.00'}</td>
                  <td className="p-2 border-r border-gray-300">{i===1?'₹14.29':i===2?'₹150.00':i===3?'₹1,126.00':'₹120.00'}</td>
                  <td className="p-2 border-r border-gray-300">{i===1?'0.29':'0'}</td>
                  <td className="p-2 border-r border-gray-300">0</td>
                  <td className="p-2 border-r border-gray-300">0.00</td>
                  <td className="p-2 border-r border-gray-300">{i===1?'₹140.00':i===2?'₹1,500.00':i===3?'₹5,630.00':'₹1,200.00'}</td>
                  <td className="p-2">{i===1?'₹140.00':i===2?'₹1,500.00':i===3?'₹5,630.00':'₹1,200.00'}</td>
                </tr>
              ))
            )}
            ...
            <tr className="text-white font-bold" style={{ backgroundColor: primaryColor }}>
              ...
              <td className="p-2 border-r border-white/20">{totalQty || '35.00'}</td>
              <td colSpan={5} className="border-r border-white/20"></td>
              <td className="p-2 border-r border-white/20">₹{totalTaxable || '8,472.90'}</td>
              <td className="p-2">₹{totalFinal || '8,472.90'}</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div className="flex-[2] pr-4 flex flex-col gap-4">
          <div>
            <div className="font-bold flex gap-2">
              ...
              <span style={{ color: primaryColor }}>Nine Thousand Two Hundred and Seventy Rupees Only</span>
            </div>
            <div className="text-[9px] text-gray-500 mt-1">Payment Details: 02-09-2025 - 310 - Default Bank</div>
          </div>
          ...
          <div className="flex gap-4 items-center">
            {footerSettings?.showQrCode && qrCodeUrl && (
              <div className="flex flex-col items-center">
                ...
                <div className="text-[10px] mt-1 font-semibold text-center">9000000@axisbank</div>
              </div>
            )}
            ...
            {footerSettings?.showBankDetails && (
              <div className="flex flex-col gap-1 text-[10px] ml-4 font-bold">
                 ...
                 <div className="grid grid-cols-[100px_1fr]">
                   <div>Bank</div><div>: {previewInvoice?.bankDetails?.bankName || 'Axis Bank'}</div>
                 </div>
                 <div className="grid grid-cols-[100px_1fr]">
                   <div>Account No.</div><div>: {previewInvoice?.bankDetails?.accountNumber || '9674563210258'}</div>
                 </div>
                 <div className="grid grid-cols-[100px_1fr]">
                   <div>IFSC Code</div><div>: {previewInvoice?.bankDetails?.ifscCode || 'UTIB0002996'}</div>
                 </div>
                 <div className="grid grid-cols-[100px_1fr]">
                   <div>A/C Name</div><div>: {previewInvoice?.bankDetails?.accountName || 'Nishit'}</div>
                 </div>
                 <div className="grid grid-cols-[100px_1fr]">
                   <div>Branch</div><div>: {previewInvoice?.bankDetails?.branchName || 'ALTHAN'}</div>
                 </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-3 mt-4">
            <div>
              ...
              <div className="text-gray-700 leading-tight">
                {previewInvoice?.notes || 'Narration simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\\'s standard dummy text ever since the 1500s...'}
              </div>
            </div>
            <div>
              <div className="font-bold mb-1">Terms and Conditions:</div>
              <div className="text-gray-700 leading-tight">
                {previewInvoice?.terms || 'Clearly state the payment due date, which is the date by which the client must pay the invoice amount. Specify the accepted payment methods (e.g. credit card bank transfer, PayPal) and any associated fees for certain payment methods.\\n\\nOutline any late payment penalties or interest charges that will apply if the client fails to make payment on time.'}
              </div>
            </div>
          </div>
        </div>
           ...
           <table className="w-full border-collapse border border-gray-300 text-right">
             <tbody>
               <tr className="border-b border-gray-300">
                 <td className="p-2 border-r border-gray-300 font-semibold text-left">Freight Charge Income</td>
                 <td className="p-2">₹100.00</td>
               </tr>
               <tr className="border-b border-gray-300">
                 <td className="p-2 border-r border-gray-300 font-semibold text-left">Packing Charge Income</td>
                 <td className="p-2">₹100.00</td>
               </tr>
               ...
               <tr className="border-b border-gray-300 text-white font-bold" style={{ backgroundColor: primaryColor }}>
                 ...
                 <td className="p-2">₹{totalFinal || '9,270.00'}</td>
               </tr>
             </tbody>
           </table>
...
`;
  ...
  console.log('Template3 updated successfully!');
} else {
  console.log('Could not find Template3 or Template4 boundaries.');
}
