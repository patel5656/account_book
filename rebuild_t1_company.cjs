    ...
    <div className="w-full flex flex-col font-sans text-[10px] bg-white border border-gray-400 p-0 box-border h-full max-w-[210mm]">
      ...
      <div className="w-full flex flex-col border-b border-gray-400 relative">
        ...
        <div className="flex w-full p-2 h-[100px]">
          ...
          <div className="flex-1 text-center flex flex-col items-center justify-center">
            <h2 className="text-[20px] font-bold tracking-wide uppercase text-purple-800" style={{ color: primaryColor }}>{companyProfile?.name || previewInvoice?.companyName || ''}</h2>
            {(companyProfile?.address || previewInvoice?.companyAddress) && <p className="text-[9px] uppercase mt-1">{companyProfile?.address || previewInvoice?.companyAddress}</p>}
            <p className="text-[9px] mt-1">Tel : {companyProfile?.mobileNo || previewInvoice?.companyPhone || ''} | {companyProfile?.email || previewInvoice?.companyEmail || ''}</p>
            {(companyProfile?.gstin || previewInvoice?.companyGst) && <p className="text-[10px] font-bold mt-1 uppercase">GSTIN: {companyProfile?.gstin || previewInvoice?.companyGst}</p>}
          </div>
          {/* Header QR */}
      ...
      <div className="w-full flex min-h-[120px]">
         ...
         <div className="flex-1 p-2 flex flex-col items-center justify-between text-center relative">
            <div className="font-bold text-[9px]">For, {companyProfile?.name || previewInvoice?.companyName || ''}</div>
...
`;
  ...
  console.log('Template1 rebuilt successfully with companyProfile support!');
} else {
